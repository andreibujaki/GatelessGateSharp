...
#pragma unroll
	for (int i = 0; i < 16; ++i) SHA256Buf[i] = input[i];
...
#pragma unroll
	for (int i = 0; i < 3; ++i)
	{
		...
		{
#pragma unroll
			for (int i = 0; i < 11; ++i) SHA256Buf[i] = input[i + 16];
...
__kernel void search2(__global uint8 *ctx, __global uint *output, ulong target)
{
	...
	if ((((ulong)outbuf.s7 << 32) | outbuf.s6) <= target) {
		int i = atomic_inc(output + 0xFF);
		output[i] = SWAP32(gid);
		output[256 + i * 8 + 0] = SWAP32(outbuf.s0);
		output[256 + i * 8 + 1] = SWAP32(outbuf.s1);
		output[256 + i * 8 + 2] = SWAP32(outbuf.s2);
		output[256 + i * 8 + 3] = SWAP32(outbuf.s3);
		output[256 + i * 8 + 4] = SWAP32(outbuf.s4);
		output[256 + i * 8 + 5] = SWAP32(outbuf.s5);
		output[256 + i * 8 + 6] = outbuf.s6;
		output[256 + i * 8 + 7] = outbuf.s7;
	}
}

__attribute__((reqd_work_group_size(WORKSIZE, 1, 1)))
__kernel void search_combined(__global const uint *input, __global uint *output, ulong target)
{
	// SHA256 takes 16 uints of input per block - we have 112 bytes to process
	// 8 * 16 == 64, meaning two block transforms.
