    ...
    public class OpenCLDevice : Device
    {
            ...
            : base(aDeviceIndex)
        {
            ...
            if (GetVendor() == "AMD") {
                ...
                else if (mName == "Radeon HD 7800" && mComputeDevice.MaxComputeUnits == 24) { mName = "Radeon HD 7870XT"; }
        ...
        static int sNumADLAdapters = 0;
        static ADLAdapterInfoArray sADLAdapterInfo = new ADLAdapterInfoArray();
        ...
        public static bool InitializeADL(OpenCLDevice[] mDevices) {
            ...
            if (ADL.ADL_SUCCESS == ADL.ADL2_Main_Control_Create(ADL.ADL_Main_Memory_Alloc, 1, ref ADL2Context)
                ...
                ADL.ADL_Adapter_NumberOfAdapters_Get(ref sNumADLAdapters);
                MainForm.Logger("Number of ADL Adapters: " + sNumADLAdapters.ToString());

                if (0 < sNumADLAdapters) {
                    if (null == ADL.ADL_Adapter_AdapterInfo_Get) {
                        MainForm.Logger("ADL.ADL_Adapter_AdapterInfo_Get() is not available.");
                    } else {
                        var AdapterBuffer = IntPtr.Zero;
                        var size = Marshal.SizeOf(sADLAdapterInfo);
                        AdapterBuffer = Marshal.AllocCoTaskMem((int)size);
                        Marshal.StructureToPtr(sADLAdapterInfo, AdapterBuffer, false);

                        ADLRet = ADL.ADL_Adapter_AdapterInfo_Get(AdapterBuffer, size);
                        if (ADL.ADL_SUCCESS == ADLRet) {
                            sADLAdapterInfo = (ADLAdapterInfoArray)Marshal.PtrToStructure(AdapterBuffer, sADLAdapterInfo.GetType());
                            ...
                            if (adrenalineWorkaroundRequired) {
                                ...
                                foreach (var device in mDevices) {
                                    ...
                                    if (device.GetVendor() == "AMD") {
                                        ...
                                        int boardCounter = 0;
                                        for (var i = 0; i < sNumADLAdapters; ++i) {
                                            if (sADLAdapterInfo.ADLAdapterInfo[i].AdapterName == boardName && !taken.Contains(sADLAdapterInfo.ADLAdapterInfo[i].BusNumber))
                                                boardCounter++;
                                            while (i + 1 < sNumADLAdapters && sADLAdapterInfo.ADLAdapterInfo[i].BusNumber == sADLAdapterInfo.ADLAdapterInfo[i + 1].BusNumber)
                                                ++i;
                                        }
                                        if (boardCounter <= 1) {
                                            for (var i = 0; i < sNumADLAdapters; ++i) {
                                                if (sADLAdapterInfo.ADLAdapterInfo[i].AdapterName == boardName && !taken.Contains(sADLAdapterInfo.ADLAdapterInfo[i].BusNumber)) {
                                                    ...
                                                    taken.Add(sADLAdapterInfo.ADLAdapterInfo[i].BusNumber);
                                                    break;
                                                }
                                                while (i + 1 < sNumADLAdapters && sADLAdapterInfo.ADLAdapterInfo[i].BusNumber == sADLAdapterInfo.ADLAdapterInfo[i + 1].BusNumber)
                                                    ++i;
                                            ...
                                            int[] activityTotalArray = new int[sNumADLAdapters];
                                            for (var i = 0; i < sNumADLAdapters; ++i)
                                            ...
                                            taken.Add(sADLAdapterInfo.ADLAdapterInfo[candidate].BusNumber);
                                    ...
                                    if (device.GetVendor() == "AMD") {
                                        for (var i = 0; i < sNumADLAdapters; i++) {
                                            ...
                                            if (null != ADL.ADL_Adapter_Active_Get)
                                                ADLRet = ADL.ADL_Adapter_Active_Get(sADLAdapterInfo.ADLAdapterInfo[i].AdapterIndex, ref IsActive);
                                            if (sADLAdapterInfo.ADLAdapterInfo[i].BusNumber == openclDevice.PciBusIdAMD
                                                && (device.ADLAdapterIndex < 0 || IsActive != 0)) {
                                                device.ADLAdapterIndex = sADLAdapterInfo.ADLAdapterInfo[i].AdapterIndex;
                                            }
                                        }
                                    }
                                }
        ...
        public int PowerLimit {
            ...
            set {
                ...
                ADL.ADL_Overdrive5_PowerControl_Set(ADLAdapterIndex, value - 100);
            }
        }

        public int DefaultPowerLimit {
            get {
                if (ADLAdapterIndex < 0 || null == ADL.ADL_Overdrive5_Temperature_Get)
                    return -1;
        ...
        public override void ResetFanControlSettings() {
            if (ADLAdapterIndex < 0 || null == ADL.ADL_Overdrive5_FanSpeedToDefault_Set)
                return;
            ADL.ADL_Overdrive5_FanSpeedToDefault_Set(ADLAdapterIndex, 0);
        }
        ...
        public override void ResetOverclockingSettings() {
            int ret;
            ...
            var ODNLevelsBuffer = Marshal.AllocCoTaskMem((int)ODNSize);
        ...
        public int CoreClock {
            ...
            set {
                ...
                if (!reset) {
                    ...
                    for (int i = 1; i < OSADLODNPerformanceLevelsData.iNumberOfPerformanceLevels; ++i) {
                        if (i != sourceIndex) {
                            OSADLODNPerformanceLevelsData.aLevels[i] = OSADLODNPerformanceLevelsData.aLevels[sourceIndex];
        ...
        public int MemoryClockStep {
            get {
                // OverDrive 5
                ADLODParameters OSADLParametersData = new ADLODParameters();
            ...
            set {
                ...
                if ((ret = ADL.ADL_Overdrive5_ODPerformanceLevels_Get(ADLAdapterIndex, reset ? 1 : 0, levelsBuffer)) == ADL.ADL_SUCCESS) {
                    ...
                    if (!reset) {
                        for (int i = 1; i < ADL.ADL_MAX_NUM_PERFORMANCE_LEVELS_OD5; ++i) {
                            OSADLODPerformanceLevelsData.aLevels[i] = OSADLODPerformanceLevelsData.aLevels[ADL.ADL_MAX_NUM_PERFORMANCE_LEVELS_OD5 - 1];
                            OSADLODPerformanceLevelsData.aLevels[i].iVddc = value;
                        }
                    }
                    Marshal.StructureToPtr(OSADLODPerformanceLevelsData, levelsBuffer, false);
                    Marshal.StructureToPtr(OSADLODPerformanceLevelsData, levelsBuffer, false);
                ...
                if (!reset) {
                    ...
                    for (int i = 1; i < OSADLODNPerformanceLevelsData.iNumberOfPerformanceLevels; ++i) {
                        if (i != sourceIndex) {
                            OSADLODNPerformanceLevelsData.aLevels[i] = OSADLODNPerformanceLevelsData.aLevels[sourceIndex];
                            OSADLODNPerformanceLevelsData.aLevels[i].iVddc = value;
                        }
                    }
                }
                Marshal.StructureToPtr(OSADLODNPerformanceLevelsData, ODNLevelsBuffer, false);
        ...
        public int MemoryVoltage {
            get {
                ...
                var size = Marshal.SizeOf(OSODNPerformanceStatusData);
